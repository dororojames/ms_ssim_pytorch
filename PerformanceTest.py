...
def SimpleTest():
    ...
    ssim = SSIM(data_range=1.).cuda()
    ms_ssim = MS_SSIM(data_range=1.).cuda()

    for name, criterion in [("ssim", ssim), ("ms_ssim", ms_ssim)]:
        loss = criterion(img1, img2).mean()
        print(name+":")
        print(loss.item())
...
def Training(t_im, ssim_type, save_video=False, video_ext="gif"):
    ...
    if ssim_type == "SSIM":
        criterion = SSIM(data_range=1., channel=t_im.size(1)).cuda()
    else:
        criterion = MS_SSIM(data_range=1., channel=t_im.size(1)).cuda()
...
def PerformanceTesting(criterion):
...
def PerformanceCompare(ssim_type="SSIM"):
    ...
    print()
    if ssim_type == "SSIM":
        criterion1 = ssim2.SSIM(window_size=11, size_average=False)
        criterion2 = ssim3.SSIM(win_size=11, win_sigma=1.5, data_range=1.,
                                size_average=False, channel=3)
        reference = ssim4.SSIM(data_range=1.)
        ours = SSIM(data_range=1.)
        names = ["ssim2", "ssim3", "reference", "ours"]
    else:
        criterion1 = ssim1.MS_SSIM(size_average=False, max_val=1.)
        criterion2 = ssim3.MS_SSIM(win_size=11, win_sigma=1.5,
                                   data_range=1., size_average=False, channel=3)
        reference = ssim4.MS_SSIM(data_range=1.)
        ours = MS_SSIM(data_range=1.)
        names = ["ssim1", "ssim3", "reference", "ours"]

    for idx, criterion in enumerate([criterion1, criterion2, reference, ours]):
        print('Testing %s' % names[idx])
        PerformanceTesting(criterion.cuda())
        print()
